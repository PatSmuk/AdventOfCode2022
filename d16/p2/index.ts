...
const DO_NOTHING = Symbol('do nothing')
type Choice = string | typeof OPEN_CURRENT_VALVE | typeof DO_NOTHING
...
function calculatePressureReliefUpperBound(state: State): number {
  // Check cache.
  const key = `${state.myCurrentValve}/${state.elephantCurrentValve}/${state.openValves.join(',')}`
  ...
  const myTimeToNextValve = calculateMinTimeToReachNextValve(state.myCurrentValve, state.openValves)
  const elephantTimeToNextValve = calculateMinTimeToReachNextValve(state.elephantCurrentValve, state.openValves)

  let minute = 0
  let pressureRelieved = 0
  let openValvesTotalFlow = state.openValvesTotalFlow
  let myValvesReached = 0
  let elephantValvesReached = 0
  let myMinsToValve = myTimeToNextValve[myValvesReached++]
  let elephantMinsToValve = elephantTimeToNextValve[elephantValvesReached++]
  ...
  while (closedValves.length > 0 && minute < MAX_MINUTE) {
    ...
    if (myMinsToValve > 0) {
      ...
      myMinsToValve = myTimeToNextValve[myValvesReached++]

      // Check if we are done before checking elephant.
      if (closedValves.length === 0) {
        break
      }
    }

    if (elephantMinsToValve > 0) {
      elephantMinsToValve--
    } else {
      const openedValve = closedValves.shift()!
      openValvesTotalFlow += valveFlowRates[openedValve]
      elephantMinsToValve = elephantTimeToNextValve[elephantValvesReached++]
    }
  }
...
if (isMainThread) {
  ...
  while (statesToVisit.length > 0) {
    ...
    } else if (state.myChoice !== DO_NOTHING) {
      state.myCurrentValve = state.myChoice
    }

    // Elephant does its choice.
    if (state.elephantChoice === OPEN_CURRENT_VALVE) {
      state.openValves = [...state.openValves, state.elephantCurrentValve]
      state.openValves.sort()
      state.openValvesTotalFlow += valveFlowRates[state.elephantCurrentValve]
    } else if (state.elephantChoice !== DO_NOTHING) {
      state.elephantCurrentValve = state.elephantChoice
    }

    const myChoices = [] as Choice[]

    // If we can still do anything...
    if (state.myChoice !== DO_NOTHING) {
    ...
    const elephantChoices = [] as Choice[]

    // Same as above but for elephant.
    if (state.elephantChoice !== DO_NOTHING) {
      const neighbours = valveNeighbours[state.elephantCurrentValve]
      const closedValves = valvePriority.filter(v => v !== state.elephantCurrentValve && !state.openValves.includes(v))

      if (closedValves.length > 0) {
        const distanceToClosedValves = closedValves.map(v => distanceToValve[state.elephantCurrentValve][v])

        for (const neighbour of neighbours) {
          const neighbourDistances = distanceToValve[neighbour]
          if (distanceToClosedValves.some((d, i) => closedValves[i] === neighbour || d > neighbourDistances[closedValves[i]])) {
            elephantChoices.push(neighbour)
          }
        }
      }

      if (!state.openValves.includes(state.elephantCurrentValve) && valveFlowRates[state.elephantCurrentValve] > 0) {
        elephantChoices.push(OPEN_CURRENT_VALVE)
      }
    }

    // If both of us are out of things to do, stop here.
    if (myChoices.length === 0 && elephantChoices.length === 0) {
    ...
    // If we can't do anything but elephant can, we do nothing next turn.
    if (myChoices.length === 0) {
      myChoices.push(DO_NOTHING)
    }
    // And vice-versa.
    if (elephantChoices.length === 0) {
      elephantChoices.push(DO_NOTHING)
    }

    // For each combination of our choices...
    for (const myChoice of myChoices) {
      for (const elephantChoice of elephantChoices) {
        // Do not allow me and the elephant to open the same valve at the same time.
        if (state.myCurrentValve === state.elephantCurrentValve && myChoice === OPEN_CURRENT_VALVE && elephantChoice === OPEN_CURRENT_VALVE) {
          continue
        }
